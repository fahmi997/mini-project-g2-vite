      ...
      return (
        <>
          {dummyEventData.map((event, index) => (
            <Card
              key={index} // Diberikan key yang unik
              minWidth={"230px"}
              maxWidth={"230px"}
              height={"270px"}
              boxShadow={"md"}
              ...
              mt={"5"}
            >
              <Image height={'115px'} width={'230px'} objectFit={'cover'} src={event.image} />
    
              <Box ml={"3"}>
                <Text textOverflow={'ellipsis'} whiteSpace={'nowrap'} overflow='hidden' mt={"5"} mr={'4'}>{event.eventName}</Text>
                <Text textOverflow={'ellipsis'} whiteSpace={'nowrap'} overflow='hidden' fontWeight={'light'} mt={"1"} mr={'4'}>{event.eventDate}</Text>
                <Text fontWeight={'bold'} mt={"1"} mr={'4'}>{event.eventPrice}</Text>
                <Box mt={"3"} bgColor={"black"} width={"auto"} border={"1px #DCDCDC solid"} mr={"3"} />
                <Text textOverflow={'ellipsis'} whiteSpace={'nowrap'} overflow='hidden' ml={"1"} mt={"2"} mr={'4'}>{event.creatorEvent}</Text>
              </Box>
            </Card>
          ))}
        </>
